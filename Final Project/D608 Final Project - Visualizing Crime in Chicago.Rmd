...
homicides_df <-
  ...
  arrange(Year, Month) %>% 
  summarise(Reported_homicides = sum(ifelse(Crime.Type == "Violent", 1, 0))) 

homicides_ts <- ts(homicides_df$Reported_homicides, start=c(2001, 1), end=c(2017, 12), frequency=12)    



ggseasonplot(homicides_ts, year.labels=TRUE, year.labels.left=TRUE) +
